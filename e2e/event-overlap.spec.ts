...
test.describe('일정 겹침 처리', () => {
  ...
  test('일정 생성 시 부분적으로 겹치는 일정이 있으면 겹침 경고 다이얼로그가 표시된다', async ({
    page,
  }) => {
    // Arrange: 기존 일정 생성
    await page.getByLabel('제목').fill('기존 회의');
    ...
    // Act: 부분적으로 겹치는 일정 생성 시도 (15:00-17:00)
    await page.getByLabel('제목').fill('부분 겹침 회의');
    await page.getByLabel('날짜').fill('2025-10-21');
    await page.getByLabel('시작 시간').fill('15:00');
    await page.getByLabel('종료 시간').fill('17:00');
    await page.getByLabel('설명').fill('부분 겹침');
    await page.getByLabel('위치').fill('회의실');
    await page.getByLabel('카테고리').click();
    await page.getByRole('option', { name: '업무-option' }).click();
    await page.getByTestId('event-submit-button').click();
  ...
  test('일정 생성 시 시작 시간이 겹치면 겹침 경고가 표시된다', async ({ page }) => {
    // Arrange: 기존 일정 생성 (14:00-15:00)
    await page.getByLabel('제목').fill('기존 회의');
    ...
    // Act: 시작 시간이 겹치는 일정 생성 (14:00-16:00)
    await page.getByLabel('제목').fill('시작 시간 겹침');
    await page.getByLabel('날짜').fill('2025-10-22');
    await page.getByLabel('시작 시간').fill('14:00');
    await page.getByLabel('종료 시간').fill('16:00');
    await page.getByLabel('설명').fill('시작 시간 겹침');
    await page.getByLabel('위치').fill('회의실');
    await page.getByLabel('카테고리').click();
    await page.getByRole('option', { name: '업무-option' }).click();
    await page.getByTestId('event-submit-button').click();
  ...
  test('일정 생성 시 종료 시간이 겹치면 겹침 경고가 표시된다', async ({ page }) => {
    // Arrange: 기존 일정 생성 (14:00-15:00)
    await page.getByLabel('제목').fill('기존 회의');
    ...
    // Act: 종료 시간이 겹치는 일정 생성 (13:00-15:00)
    await page.getByLabel('제목').fill('종료 시간 겹침');
    await page.getByLabel('날짜').fill('2025-10-23');
    await page.getByLabel('시작 시간').fill('13:00');
    await page.getByLabel('종료 시간').fill('15:00');
    await page.getByLabel('설명').fill('종료 시간 겹침');
    await page.getByLabel('위치').fill('회의실');
    await page.getByLabel('카테고리').click();
    await page.getByRole('option', { name: '업무-option' }).click();
    await page.getByTestId('event-submit-button').click();
  ...
  test('일정 생성 시 여러 일정과 겹치면 모든 겹치는 일정이 다이얼로그에 표시된다', async ({
    ...
    // Arrange: 첫 번째 기존 일정 생성 (14:00-15:00)
    ...
    // Act: 여러 일정과 겹치는 일정 생성 (14:30-15:30)
    await page.getByLabel('제목').fill('여러 겹침 회의');
    await page.getByLabel('날짜').fill('2025-10-25');
    await page.getByLabel('시작 시간').fill('14:30');
    await page.getByLabel('종료 시간').fill('15:30');
    await page.getByLabel('설명').fill('여러 겹침');
    await page.getByLabel('위치').fill('회의실');
    await page.getByLabel('카테고리').click();
    await page.getByRole('option', { name: '업무-option' }).click();
    await page.getByTestId('event-submit-button').click();

    // Assert: 겹침 경고 표시 및 모든 겹치는 일정 표시
    await expect(page.getByText('일정 겹침 경고')).toBeVisible({ timeout: 10000 });
    await expect(
      page
        .getByText('첫 번째 회의')
        .filter({ hasText: /^첫 번째 회의$/ })
        .first()
    ).toBeVisible();
    await expect(
      page
    ...
    // await expect(page.getByText(/첫 번째 회의/)).toBeVisible();
    // await expect(page.getByText(/두 번째 회의/)).toBeVisible();
  });
  ...
  test('일정 수정 시 겹침 경고 후 취소를 선택하면 일정이 수정되지 않는다', async ({ page }) => {
    // Arrange: 첫 번째 일정 생성
    await page.getByLabel('제목').fill('수정 취소 테스트');
    await page.getByLabel('날짜').fill('2025-10-30');
    await page.getByLabel('시작 시간').fill('10:00');
    await page.getByLabel('종료 시간').fill('11:00');
    await page.getByLabel('설명').fill('수정 취소');
    await page.getByLabel('위치').fill('회의실');
    await page.getByLabel('카테고리').click();
    await page.getByRole('option', { name: '업무-option' }).click();
    await page.getByTestId('event-submit-button').click();
  ...
  // 엣지 케이스 테스트
  test('정확히 같은 시작 시간과 종료 시간을 가진 일정은 겹침으로 간주된다', async ({ page }) => {
    // Arrange: 기존 일정 생성
    await page.getByLabel('제목').fill('기존 회의');
    await page.getByLabel('날짜').fill('2025-10-31');
    await page.getByLabel('시작 시간').fill('14:00');
    await page.getByLabel('종료 시간').fill('15:00');
    await page.getByLabel('설명').fill('기존 일정');
    await page.getByLabel('위치').fill('회의실');
    ...
    // Act: 정확히 같은 시간대의 일정 생성 시도
    await page.getByLabel('제목').fill('같은 시간 회의');
    await page.getByLabel('날짜').fill('2025-10-31');
    await page.getByLabel('시작 시간').fill('14:00');
    await page.getByLabel('종료 시간').fill('15:00');
    await page.getByLabel('설명').fill('같은 시간');
    await page.getByLabel('위치').fill('회의실');
    await page.getByLabel('카테고리').click();
    await page.getByRole('option', { name: '업무-option' }).click();
    await page.getByTestId('event-submit-button').click();
  ...
  test('다른 날짜의 일정은 겹치지 않는다', async ({ page }) => {
    // Arrange: 첫 번째 일정 생성 (2025-10-20 14:00-15:00)
    await page.getByLabel('제목').fill('첫 번째 회의');
    await page.getByLabel('날짜').fill('2025-10-20');
    await page.getByLabel('시작 시간').fill('14:00');
    await page.getByLabel('종료 시간').fill('15:00');
    ...
    // Act: 같은 시간대이지만 다른 날짜의 일정 생성 (2025-10-21 14:00-15:00)
    await page.getByLabel('제목').fill('두 번째 회의');
    await page.getByLabel('날짜').fill('2025-10-21');
    await page.getByLabel('시작 시간').fill('14:00');
    await page.getByLabel('종료 시간').fill('15:00');
